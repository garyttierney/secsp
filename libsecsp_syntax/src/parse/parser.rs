...
pub fn parse_path() {
    ...
    let mut parser = parser_from_source(&sess, "first.second.third").expect("couldn't tokenize");
    let path = parser.parse_path().expect("unable to parse");
    let names: Vec<String> = path.segments.iter().map(|n| n.value.clone()).collect();

    assert_eq!(ByteSpan::new(ByteIndex(0), ByteIndex(17)), path.span);
    assert_eq!(vec!["first", "second", "third"], names);
}

#[test]
fn parse_abstract_block() {
    let sess = ParseSession::default();
    let mut parser = parser_from_source(&sess, "abstract block abc inherits_from abc {}")
        .expect("couldn't tokenize");
    let stmt = parser.parse_statement().expect("unable to parse");

    assert_eq!(
        StatementKind::ContainerDeclaration(
            ContainerType::Block(true),
            Ident {
                value: "abc".to_owned(),
                span: ByteSpan::new(ByteIndex(15), ByteIndex(17)),
            },
            vec![],
            vec![],
        ),
...
fn parse_inner_block() {
    let sess = ParseSession::default();
    let mut parser = parser_from_source(&sess, "abstract block abc { block abc { type t; } }")
        .expect("couldn't tokenize");
    let stmt = parser.parse_statement().expect("unable to parse");
